...
class Pattern:
    ...
    def sense_self_and_other(self, field, t):
        ...
        затем три более ранних измерения опыта (24.07).
        ...
        0) Global Workspace (workspace/workspace_weights) — единый буфер,
           ...
           внимание: чем сильнее канал изменился с прошлого тика
           ("неожиданность"), тем больше его вес в этом тике — это ещё не
           замена существующей логике принятия решений (это отдельный,
           более рискованный шаг на будущее), но единое представление
           "я сейчас", которое уже сегодня питает аффективный слой и
           felt_state, а завтра — внутреннюю речь и остальные пункты плана.
        ...
        1) Ощущение (felt_state) — то же самое поле воспринимается
           по-разному в зависимости от внутреннего состояния, ТЕПЕРЬ ещё и
           окрашено valence (позитивное настроение усиливает сигнал поля,
           негативное — приглушает, как и задумано в исходном плане).
        ...
        3) Чужой разум (_mind_prediction / _social_surprise) — прогноз
           ...
           unresolved_contradiction, а не гасится как шум.
        """
        # --- 0) Global Workspace ---
        ...
        # определяет вес канала в этом тике — громче не тот, кто сильнее,
        # а тот, кто НЕОЖИДАННЕЕ.
        surprise_weights = {}
        for k, v in raw_workspace.items():
            prev = self._workspace_prev.get(k, v)
            surprise_weights[k] = abs(v - prev)
        total_surprise = sum(surprise_weights.values()) or 1e-6
        self.workspace_weights = {k: w / total_surprise for k, w in surprise_weights.items()}
