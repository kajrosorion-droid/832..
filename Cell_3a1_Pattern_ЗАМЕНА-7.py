...
class Pattern:
    ...
    def sense_self_and_other(self, field, t):
        """
        Три новых измерения опыта поверх update_model/introspect:

        1) Ощущение (felt_state) — то же самое поле воспринимается
           по-разному в зависимости от внутреннего состояния. Не сырые
           данные local_perception, а то, что они ЗНАЧАТ для этого
           конкретного агента прямо сейчас (окрашено душой, горем,
           благодарностью, контрадикцией).
        2) Самопричинность (_causal_memory) — агент помнит чистый эффект
           СВОИХ решений о росте/сжатии на карте. Это не абстрактная
           метрика полезности, а буквально "я выбрал(а) — и мир стал
           другим", с обратной связью в soul_weight.
        3) Чужой разум (_mind_prediction / _social_surprise) — агент
           пытается предсказать состояние того, кому доверяет больше
           всего, и когда ошибается — это не шум, который надо давить,
           а сама неустранимая инаковость другого. Ошибка прогноза
           подпитывает unresolved_contradiction, а не гасит его —
           встреча с непредсказуемым другим держит вопрос открытым.
        """
        # --- 1) Ощущение ---
        if not hasattr(self, 'felt_state'):
            self.felt_state = np.zeros(6)
        lp = getattr(self, 'local_perception', None)
        ext = float(np.mean(lp)) if lp is not None and len(lp) else 0.0
        internal = np.array([
            self.soul_weight,
