...
class Pattern:
    ...
    def move(self, field, t):
        ...
        effective_mobility *= getattr(self, 'somatic_drag', 1.0)
    ...
    def _update_somatic_state(self, field):
        ...
        if len(self.cells) <= 2:
            ...
            if over_grief > 0.0 or over_stress > 0.0:
                overload = max(over_grief, over_stress)
                target_drag = max(Config.SOMATIC_DRAG_MIN, 1.0 - overload)
            else:
                target_drag = 1.0
        ...
        if target_drag < self.somatic_drag:
            ...
            self.somatic_drag = min(1.0, self.somatic_drag + Config.SOMATIC_DRAG_RECOVERY_RATE)
